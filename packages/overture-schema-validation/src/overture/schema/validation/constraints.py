    ...
    def validate(self, value: BaseModel, info: ValidationInfo) -> None:
        ...
        if hasattr(value, "__pydantic_extra__") and value.__pydantic_extra__:
            for field_name in value.__pydantic_extra__.keys():
                if not any(
                    field_name.startswith(prefix) for prefix in self.allowed_prefixes
                ):
                    context = info.context or {}
                    ...
                    raise ValidationError.from_exception_data(
                        ...
                        line_errors=[
                            InitErrorDetails(
                                ...
                                input=value,
                                ctx={
                                    "error": f"Unrecognized field '{field_name}' must use one of these prefixes: {sorted(self.allowed_prefixes)}"
                                },
                            )
                        ],
                    )
