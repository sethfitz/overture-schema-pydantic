...
3. ✅ **Check validation approach** - Infrastructure now enforces per-theme-type schema
   - Per-theme-type schema generation is automatic and required
   - Focus on constraint implementation issues
...
- ✅ **Infrastructure enforces per-theme-type approach** (no monolithic fallback)

## Current Status

✅ **Infrastructure Complete:**

- Schema validation: All 15 per-theme-type schemas generate and validate correctly
- Examples validation: All 100+ examples pass using correct schemas
- Content-based theme/type extraction works reliably
- Monolithic schema support completely removed

❌ **Constraint Validation Issues:**

- Most counterexamples still pass validation when they should fail
- Issues are in Pydantic model constraint implementations, not infrastructure
- Need to focus on constraint JSON Schema generation (`__get_json_schema__()` methods)
...
5. **Content-based extraction is essential:** Theme/type extraction from file paths is unreliable and fragile. Using `yq` to read from actual file content is much more robust.

6. **Hard failures prevent confusion:** Removing monolithic schema fallback and failing hard when theme/type extraction fails makes debugging much clearer.

7. **Model registration is critical:** All model packages must be imported in `__main__.py` for proper registration and schema generation.

## Next Steps

With infrastructure complete, focus should shift to:

1. **Constraint validation fixes:** Fix individual constraint implementations to properly generate JSON Schema
2. **Systematic counterexample testing:** Work through failing counterexamples to identify specific constraint issues
3. **Constraint coverage:** Ensure all validation constraints have corresponding JSON Schema generation
